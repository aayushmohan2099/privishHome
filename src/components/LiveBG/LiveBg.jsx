// src\components\LiveBG\LiveBg.jsx
...
// Default luxurious violet/orange blend to match Privish branding
const defaultColors = ["#FDF8F4", "#FD7B03", "#FD3502"];
...
export default function LiveBg({
  ...
  className = "",
  fixed = true, // Set to true to cover the entire viewport, false to bound it to the parent container
}) {
  ...
  useEffect(() => {
    ...
    const iMouseStrengthLocation = gl.getUniformLocation(
      program,
      "iMouseStrength",
    );
    ...
    const uNoiseIntensityLocation = gl.getUniformLocation(
      program,
      "uNoiseIntensity",
    );
    ...
    const render = () => {
      ...
      mouseStrength.value +=
        (mouseStrength.targetValue - mouseStrength.value) * 0.08;
  ...
  const finalBlurClass =
    blurClassMap[backdropBlurAmount] || blurClassMap["none"];

  return (
    <div
      ref={containerRef}
      className={`${fixed ? "fixed" : "absolute"} inset-0 w-full h-full overflow-hidden -z-10 ${className}`}
